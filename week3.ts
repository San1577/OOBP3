...
class Refrigerator {
  ...
  getContents(): string[] {
      return this.food.map(item => item.getName());
  }

  eatFood(foodName: string): string {
      const index = this.food.findIndex(item => item.getName() === foodName);
      ...
      return `You ate ${eatenFood.getName()} with ${eatenFood.calories} calories`;
  }

  getTotalCalories(): number {
      return this.food.reduce((total, item) => total + item.calories, 0);
  }
